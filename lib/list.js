...
Gun.chain.list = function (cb, opt) {
  ...
  gun.last = function (obj, cb) {
    ...
    return gun.path('last').put(null).put(obj).val(function (val) {
  ...
  gun.first = function (obj, cb) {
    ...
    return gun.path('first').put(null).put(obj).val(function () {
      // warning! these are not transactional! They could be.
...
(function () {
  ...
  setTimeout(function () {
    list.val(function (val) {
      ...
      return;
      list.path('first').val(Gun.log).path('next').val(Gun.log).path('next').val(Gun.log);
    });
  }, 1000);
