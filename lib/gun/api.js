...
function applyBack(Gun) {
  ...
  Gun.chain.Back = function (n, opt) {
    ...
    if (n instanceof Array) {
      ...
      if (u !== _tmp) return opt ? gun : _tmp;else if (_tmp = at.back) return _tmp.Back(n, opt);
      return;
    }
...
function applyGet(Gun) {
  ...
  function value(at, ev) {
    ...
    if (!(rel = Gun.val.rel.is(put))) {
      if (!Gun.val.is(put)) return false;
      not(cat, at);
      return true;
    }
    //cat.change = at.put;
...
function applyKey(Gun) {
  ...
  Gun.chain.key = function (index, cb, opt) {
    ...
    var _field = Gun._.field;
    var _sid = Gun.on.ask._;
    var _rid = Gun.on.ack._;
    if (!index) {
      if (cb) {
...
function api(Gun) {
  var applyFNs = [applyBack, applyPut, applyGet, applyAny, applyOn, applyKey, applyPath, applyVal, applyOff, applyNot, applyMap, applyInit, applySet];
  return applyFNs.reduce(function (_, applyFN) {
    applyFN(_);
    return _;
  }, Gun);
}