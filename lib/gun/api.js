...
var obj = _2.default.obj;
var obj_is = obj.is;
var obj_put = obj.put;
var obj_map = obj.map;
var obj_empty = obj.empty;
var num = _2.default.num;
var num_is = num.is;
var _soul = _2.default.val.rel._;
var _field = '.'(function () {
  var obj = {};
  var u;
  _2.default.chain.Back = function (n, opt) {
    var tmp;
    if (-1 === n || Infinity === n) return this._.root;else if (1 === n) return this._.back;
    var gun = this,
        at = gun._;
    ...
    if (n instanceof Array) {
      ...
      if (u !== tmp) return opt ? gun : tmp;else if (tmp = at.back) return tmp.Back(n, opt);
      return;
    }
  ...
  _2.default.chain.put = function (data, cb, opt, as) {
    ...
    if (root === gun || as.soul) {
      if (!obj_is(as.data)) {
        (opt.any || noop).call(opt.as || gun, as.out = { err: _2.default.log('No field to put', (0, _typeof3.default)(as.data), '"' + as.data + '" on!') });
        if (as.res) {
          as.res();
        }
        return gun;
      ...
      as.gun = gun = root.get(as.soul = as.soul || (as.not = _2.default.node.soul(as.data) || (opt.uuid || root.Back('opt.uuid') || _2.default.text.random)()));
      as.ref = as.ref || as.gun;
      ify(as);
      return gun;
    }
    if (_2.default.is(data)) {
      data.any(function (e, d, k, at, ev) {
        ev.off();
        var s = _2.default.node.soul(d);
        if (!s) {
          _2.default.log('Can only save a node, not a property.');return;
        }
        gun.put(_2.default.val.rel.ify(s), cb, opt);
      });
      return gun;
    }
    ...
    if (!as.out) {
      as.res = as.res || _2.default.on.stun(as.ref);
  ...
  function ify(as) {
    as.batch = batch;
    var opt = as.opt,
        env = as.env = _2.default.state.map(map, opt.state);
    env.soul = as.soul;
    as.graph = _2.default.graph.ify(as.data, env, as);
    if (env.err) {
      (opt.any || noop).call(opt.as || as.gun, as.out = { err: _2.default.log(env.err) });
      if (as.res) {
        as.res();
      }
      return;
  ...
  function batch() {
    ...
    as.ref.on('out', {
      ...
      '#': _2.default.on.ask(function (ack, ev) {
        if (ack && 0 === ack.ok) {
          return;
        }
  ...
  function map(v, f, n, at) {
    ...
    if (as.not || _2.default.node.soul(at.obj)) {
      at.soul(_2.default.node.soul(at.obj) || (as.opt.uuid || as.gun.Back('opt.uuid') || _2.default.text.random)());
      return;
    }
  ...
  function soul(at, ev) {
    ...
    ev.stun(); // TODO: BUG!?
    ev.off();
    cat.soul(_2.default.node.soul(cat.obj) || _2.default.node.soul(at.put) || _2.default.val.rel.is(at.put) || (as.opt.uuid || as.gun.Back('opt.uuid') || _2.default.text.random)()); // TODO: BUG!? Do we really want the soul of the object given to us? Could that be dangerous?
  ...
  function any(at, ev) {
    ...
    var cat = as.ref._,
        ...
        tmp;
    if (u === data) {
      if (opt.init || as.gun.Back('opt.init')) return;
      /*
      ...
      */
      ...
      if (as.gun.Back(-1) !== cat.back) implicit(at);
      tmp = tmp || at.get;
      any.call(as, {
        put: as.data,
    ...
    ev.off(ev.stun());
    if (!as.not && !(as.soul = _2.default.node.soul(data))) {
      if (as.path && obj_is(as.data)) {
        // Apparently necessary
        as.soul = (opt.uuid || as.gun.Back('opt.uuid') || _2.default.text.random)();
      } else {
        /*
        ...
        */
        ...
        (as.next = as.next || _2.default.on.next(as.ref))(function (next) {
  ...
  _2.default.chain.get = function (lex, cb, opt) {
    if (!opt || !opt.path) {
      var back = this.Back(-1);
    } // TODO: CHANGING API! Remove this line!
    var gun,
        back = back || this,
        cat = back._;
    var path = cat.path || empty,
        tmp;
    if (typeof lex === 'string') {
      ...
      (gun = back.chain())._.err = { err: _2.default.log('Invalid get request!', lex) };
      if (cb) {
      ...
      if (typeof at.get === 'string') {
        ...
        if (cat.ask) {
          ...
          cat.on('in', function (tac, ev) {
            ...
            if (!(tmp = _2.default.node.soul(tmp = tac.put) || _2.default.val.rel.is(tmp))) {
        ...
        return;
      } else if (at.get instanceof Function) {
        if (!cat.ask) {
          var opt = at.opt || {};
  ...
  function input(at, ev) {
    var cat = this,
        tmp;
    cat.id = cat.id || _2.default.text.random(5); // TOD: BUG! This allows for 1B item entropy in memory. In the future, people might want to expand this to be larger.
    if (at.err) {
      console.log('Please report this as an issue! In.err'); // TODO: BUG!
      return;
    }
    console.debug(10, 'input', at, cat.get);
    if (value.call(cat, at, ev)) return;
    if (tmp = cat.link) {
      if (tmp = tmp.res) {
  ...
  _2.default.chain.get.input = input;
  function value(at, ev) {
    ...
    if (!cat.link && _2.default.node.soul(put) && (rel = _2.default.node.soul(at.put))) {
      console.debug(11, 'value', put);
      ask(cat, rel);
      return false;
    }
    if (!(rel = _2.default.val.rel.is(put))) {
      if (!_2.default.val.is(put)) return false;
      not(cat, at);
      return true;
    }
    //cat.change = at.put;
    ...
    ask(cat, rel);
    if (!tmp.ran) tmp.res(); // This is necessary for things that listen for a soul or relation only.
    return true;
  }
  function map(data, key) {
    // Map over only the changes on every update.
    if (_2.default._.meta === key) {
      return;
    }
    var cat = this.cat,
        path = cat.path || {},
    ...
    if (cat.put && obj_has(cat.put, key)) {
      data = cat.put[key];
    } // But use the actual data.
    (at = gun._).change = cat.change[key];
    at.put = data;
    if (tmp = _2.default.val.rel.is(at.put)) {
      // PERFORMANCE HACK!
      if (tmp = gun.Back(-1).get(tmp)._.put) {
        // PERFORMANCE HACK!
  ...
  function not(cat, at) {
    ...
    if (u !== cat.put) {
      cat.link = null;
    } // TODO: BUG! This may mean `not` will be fired multiple times until data is found. Is this okay?
    if (null === tmp) {
      return;
    }
    if (tmp) {
  ...
  function ask(cat, soul) {
    ...
    if (obj_has(tmp, '_')) {
      ...
      lex['#'] = _2.default.on.ask(ack, lex);
      cat.gun.on('out', lex);
      return;
    }
    // TODO: PERF! Make it so we do not have to iterate through this every time?
    obj_map(tmp, function (v, key) {
      if (!v || cat.put && cat.put[key]) {
        return;
      } // TODO: This seems like an optimization? But does it have side effects? Probably not without the tmp[key] = 0;
      if (!(v = (cat.path || empty)[key])) {
        return;
      }
      tmp[key] = 0;
  ...
  _2.default.chain.any = function (any, opt) {
    ...
    console.debug(5, 'any');
    chain.on('out', { get: function get(at, ev) {
        //console.log("any!", at);
        if (!at.gun) {
          console.log('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%EXPLODE%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%', at);
        }
        var gun = at.gun || chain,
            cat = gun._;
        var data = cat.put || at.put,
            tmp;
        if ((tmp = at.put) && tmp[_2.default.val.rel._] && (tmp = _2.default.val.rel.is(tmp))) {
          if (null !== opt['.']) {
            return;
          }
          at = obj_to(at, { put: data = cat.change = cat.put = _2.default.state.ify(_2.default.node.ify({}, tmp)) });
        }
        // TODO: BUG! Need to use at.put > cat.put for merged cache?
        if (tmp = opt.change) {
          // TODO: BUG! Opt is outer scope, gun/cat/data might be iterative and thus only inner scope? Aka, we can't use it for all of them.
          if (1 === tmp) {
            opt.change = true;
          } else {
            data = cat.change;
  ...
  var empty = {},
      u;
  var _soul = _2.default._.soul,
      _field = _2.default._.field,
      _sid = _2.default.on.ask._,
      _rid = _2.default.on.ack._;
}())(function () {
  _2.default.chain.key = function (index, cb, opt) {
    if (!index) {
      if (cb) {
        cb.call(this, { err: _2.default.log('No key!') });
      }
      return this;
    }
    var gun = this;
    ...
    if (gun === gun._.root) {
      if (cb) {
        cb({ err: _2.default.log('Can\'t do that on root instance.') });
      }return gun;
    }
    opt = opt || {};
    opt.key = index;
    ...
    gun.on(key, { as: opt });
    if (!opt.data) {
      opt.res = _2.default.on.stun(opt.ref);
    }
    return gun;
  };
  function key(at, ev) {
    var opt = this;
    ev.off();
    opt.soul = _2.default.node.soul(at.put);
    if (!opt.soul || opt.key === opt.soul) {
      return opt.data = {};
    }
    opt.data = obj_put({}, keyed._, _2.default.node.ify(obj_put({}, opt.soul, _2.default.val.rel.ify(opt.soul)), '#' + opt.key + '#'));
    if (opt.res) opt.res(1);
  ...
  keyed._ = '##';
  _2.default.on('path', function (at) {
    var gun = at.gun;
  ...
  function normalize(at) {
    var cat = this;
    if (!at.put) {
      if (at.get) search.call(cat, at);
      return;
    }
    if (at.opt && at.opt.key) return;
    var put = at.put,
        graph = cat.gun.Back(-1)._.graph;
    _2.default.graph.is(put, function (node, soul) {
      if (!_2.default.node.is(graph['#' + soul + '#'], function each(rel, id) {
        if (id !== _2.default.val.rel.is(rel)) {
          return;
        }
        if (rel = graph['#' + id + '#']) {
          _2.default.node.is(rel, each);
          return;
        }
        _2.default.node.soul.ify(rel = put[id] = _2.default.obj.copy(node), id);
      })) {
        return;
      }
      _2.default.obj.del(put, soul);
    });
  }
  function search(at) {
    var cat = this;
    var tmp;
    if (!_2.default.obj.is(at.get)) {
      return;
    }
    if (cat.pseudo) {}
    ...
    if ((tmp = at.get) && _2.default.obj.has(tmp, '.')) {
      tmp = at['#'];
      at['#'] = _2.default.on.ask(proxy);
    }
    var tried = {};
    function proxy(ack, ev) {
      ...
      var put = ack.put,
          lex = at.get;
      if (!cat.pseudo) {
        return _2.default.on.ack(tmp, ack);
      }
      if (ack.put) {
        if (!lex['.']) {
          return _2.default.on.ack(tmp, ack);
      ...
      _2.default.obj.map(cat.seen, function (ref, id) {
  ...
  function pseudo(at, ev) {
    ...
    if (cat.pseudo) {
      ...
      cat.on('in', _2.default.obj.to(at, { put: cat.put = cat.pseudo }));
      return;
    }
    if (!at.put) {
      return;
    }
    var rel = _2.default.val.rel.is(at.put[keyed._]);
    if (!rel) {
      return;
    }
    var soul = _2.default.node.soul(at.put),
        resume = ev.stun(resume),
        root = cat.gun.Back(-1),
        seen = cat.seen = {};
    cat.pseudo = cat.put = _2.default.state.ify(_2.default.node.ify({}, soul));
    root.get(rel).on(each, true);
    function each(change) {
      _2.default.node.is(change, map);
    }
    function map(rel, soul) {
      if (soul !== _2.default.val.rel.is(rel)) {
        return;
      }
      if (seen[soul]) {
        return;
    ...
    function on(put) {
      cat.pseudo = _2.default.HAM.union(cat.pseudo, put) || cat.pseudo;
...
_2.default.chain.path = function (field, cb, opt) {
  var back = this,
      gun = back,
      tmp;
  ...
  _2.default.chain.on = function (tag, arg, eas, as) {
    var gun = this,
        at = gun._,
        tmp;
    if (!at.on) {
  ...
  _2.default.chain.val = function (cb, opt) {
    var gun = this,
        at = gun._,
        value = at.put;
    if (!at.stun && u !== value) {
  ...
  var obj = _2.default.obj,
      ...
      obj_del = obj.del,
      obj_to = obj.to;
  var val_rel_is = _2.default.val.rel.is;
  var empty = {},
      u;
})()(function () {
  _2.default.chain.not = function (cb, opt, t) {
    var gun = this,
        at = _2.default.obj.to(gun._, { not: { not: cb } });
    gun.any(ought, { as: at });
    return gun;
  };
  ...
  _2.default.chain.map = function (cb, opt, t) {
    var gun = this,
        cat = gun._,
        chain = cat.map;
    if (!chain) {
  ...
  var obj_map = _2.default.obj.map,
      noop = function noop() {},
      event = { stun: noop, off: noop },
      n_ = _2.default.node._;
}())(function () {
  _2.default.chain.init = function () {
    // TODO: DEPRECATE?
    (this._.opt = this._.opt || {}).init = true;
    return this.Back(-1).put(_2.default.node.ify({}, this._.get), null, this._.get);
  };
}())(function () {
  _2.default.chain.set = function (item, cb, opt) {
    var gun = this;
    cb = cb || function () {};
    return item.val(function (node) {
      var put = {},
          soul = _2.default.node.soul(node);
      if (!soul) {
        return cb.call(gun, { err: _2.default.log('Only a node can be linked! Not "' + node + '"!') });
      }
      gun.put(_2.default.obj.put(put, soul, _2.default.val.rel.ify(soul)), cb, opt);
    });
  };
}());