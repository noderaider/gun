...
(function (wsp) {
  ...
  */
  _gun2.default.on('opt', function (at) {
    ...
    var opt = at.opt;
    gun.__ = at.root._;
    var optWS = gun.__.opt.ws || opt.ws || {};
    opt.ws = optWS;
    gun.__.opt.ws = optWS;
    console.warn('HERE');
    function start(server) {
      var port = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 80;
      var app = arguments[2];

      if (app && app.use) app.use(gun.wsp.server);
      server = gun.__.opt.ws.server = gun.__.opt.ws.server || opt.ws.server || server;

      (0, _ws4.default)(gun.wsp.ws = gun.wsp.ws || new _ws2.Server(gun.__.opt.ws), function (req, res) {
        var ws = this;
        ...
        ws.sub = ws.sub || gun.wsp.on('network', function (msg, ev) {
          ...
          if (!msg || msg.headers && msg.headers['gun-sid'] === ws.sid) {
            return;
          }
          if (msg && msg.headers) {
          ...
          try {
            ws.send(_gun2.default.text.ify(msg));
          } catch (e) {
            console.error('SWALLOWING2', e);
            // juuuust in case.
          }
        });
    ...
    var wsp = gun.wsp = gun.wsp || function (server) {
      ...
      if (_gun2.default.fns.is(server.address)) {
        if (server.address()) {
          start(server, server.address().port);
          return gun;
        }
      }
      if (_gun2.default.fns.is(server.get) && server.get('port')) {
    ...
    gun.wsp.on = gun.wsp.on || _gun2.default.on;
    ...
    gun.wsp.server = gun.wsp.server || function (req, res, next) {
      // http
      next = next || function () {};
      if (!req || !res) {
        return next(), false;
      ...
      if (msg.url.pathname.replace(gun.wsp.regex, '').slice(0, 3).toLowerCase() === '.js') {
        ...
        res.end(gun.wsp.js = gun.wsp.js || _fs2.default.readFileSync(__dirname + '/../gun.js')); // gun server is caching the gun library for the client
        return true;
      }
      return (0, _http2.default)(req, res, function (req, res) {
        if (!req) {
          return next();
        }
        var stream,
            cb = res = (0, _jsonp2.default)(req, res);
        if (req.headers && (stream = req.headers['gun-sid'])) {
          ...
          stream.drain = stream.drain || function (res) {
            if (!res || !stream || !stream.queue || !stream.queue.length) {
              return;
            }
            res({ headers: { 'gun-sid': stream.sid }, body: stream.queue });
            stream.off = setTimeout(function () {
              stream = null;
            }, gun.wsp.pull);
          ...
          stream.sub = stream.sub || gun.wsp.on('network', function (req, ev) {
            ...
            if (!req || req.headers && req.headers['gun-sid'] === stream.sid) {
              return;
            }
          ...
          cb = function cb(r) {
            (r.headers || {}).poll = gun.wsp.poll;res(r);
          };
    ...
    if ((gun.__.opt.maxSockets = opt.maxSockets || gun.__.opt.maxSockets) !== false) {
      _https2.default.globalAgent.maxSockets = _http2.default.globalAgent.maxSockets = gun.__.opt.maxSockets || Infinity;
    }
    gun.wsp.msg = gun.wsp.msg || function (id) {
      if (!id) {
        return gun.wsp.msg.debounce[id = _gun2.default.text.random(9)] = _gun2.default.time.is(), id;
      }
      clearTimeout(gun.wsp.msg.clear);
      gun.wsp.msg.clear = setTimeout(function () {
        var now = _gun2.default.time.is();
        _gun2.default.obj.map(gun.wsp.msg.debounce, function (t, id) {
          if (now - t < 1000 * 60 * 5) {
            return;
          }
          _gun2.default.obj.del(gun.wsp.msg.debounce, id);
        });
      }, 500);
      if (id = gun.wsp.msg.debounce[id]) {
        return gun.wsp.msg.debounce[id] = _gun2.default.time.is(), id;
      }
      gun.wsp.msg.debounce[id] = _gun2.default.time.is();
      return;
    };
    ...
    gun.wsp.wire = gun.wsp.wire || function () {
      ...
      function tran(req, res) {
        ...
        if (req.url) {
          req.url = _url2.default.format(req.url);
        }
        var msg = req.body;
        ...
        gun.wsp.on('network', _gun2.default.obj.copy(req));
        if (msg['@']) {
          return;
        } // no need to process.
      ...
      tran.get = function (req, cb) {
        ...
        gun.on('out', { gun: gun,
          get: lex,
          req: 1,
          '#': _gun2.default.on.ask(function (at, ev) {
            ev.off();
            var graph = at.put;
            return cb({ headers: reply.headers,
              body: { '#': gun.wsp.msg(),
                '@': body['#'],
                '$': graph,
                '!': at.err
      ...
      tran.put = function (req, cb) {
        ...
        var body = req.body,
            ...
            reply = { headers: { 'Content-Type': tran.json } },
            opt;
        gun.on('out', { gun: gun, put: graph, '#': _gun2.default.on.ask(function (ack, ev) {
        ...
        return;
        if (_gun2.default.is.graph(req.body)) {
          if (req.err = _gun2.default.union(gun, req.body, function (err, ctx) {
            // TODO: BUG? Probably should give me ctx.graph
            if (err) {
              return cb({ headers: reply.headers, body: { err: err || 'Union failed.' } });
            }
            var ctx = ctx || {};ctx.graph = {};
            _gun2.default.is.graph(req.body, function (node, soul) {
              ctx.graph[soul] = gun.__.graph[soul];
            });
            (gun.__.opt.wire.put || function (g, cb) {
              cb('No save.');
            })(ctx.graph, function (err, ok) {
              if (err) {
