...
_gun2.default.on('opt').event(function (gun, opt) {
  if (!opt.s3) return; // don't use S3 if it isn't specified.
  opt.s3 = opt.s3 || {};
  var s3 = gun.__.opt.s3 = gun.__.opt.s3 || (0, _aws2.default)(opt && opt.s3);
  ...
  s3.get = s3.get || function (key, cb, opt) {
    ...
    if (key[_gun2.default._.soul]) {
      key = s3.prefix + s3.prenode + _gun2.default.is.soul(key);
    } else {
      key = s3.prefix + s3.prekey + key;
    }
    s3.GET(key, function (err, data, text, meta) {
      _gun2.default.log('via s3', key, err);
      ...
      if (meta && (meta.key || meta[_gun2.default._.soul])) {
        if (err) {
          return cb(err);
        }
        if (meta.key && _gun2.default.obj.is(data) && !_gun2.default.is.node(data)) {
          return _gun2.default.obj.map(data, function (rel, soul) {
            if (!(soul = _gun2.default.is.soul(rel))) {
              return;
            }
        ...
        if (meta[_gun2.default._.soul]) {
          return s3.get(meta, cb); // TODO: way faster if you use cache.
        }
        return cb({ err: _gun2.default.log('Cannot determine S3 key data!') });
      }
      if (data) {
        meta.soul = _gun2.default.is.soul.on(data);
        if (!meta.soul) {
          err = { err: _gun2.default.log('No soul on node S3 data!') };
        }
      } else {
        return cb(err, null);
      }
      ...
      cb(null, graph);
      (graph = {})[meta.soul] = _gun2.default.union.pseudo(meta.soul);
      cb(null, graph);
      if (_gun2.default.obj.map(opt.ctx.load, function (loaded, soul) {
        if (!loaded) {
          return true;
        }
      })) {
        return;
      } // return IF we have nodes still loading.
      cb(null, {});
    });
  };
  s3.put = s3.put || function (nodes, cb) {
    ...
    cb.count = 0;
    var next = s3.next,
        ack = _gun2.default.text.random(8),
        batch = s3.batch[next] = s3.batch[next] || {};
    s3.on(ack).once(cb);
    _gun2.default.obj.map(nodes, function (node, soul) {
      cb.count += 1;
  ...
  s3.put.now = s3.put.now || function () {
    ...
    s3.next = _gun2.default.time.is();
    _gun2.default.obj.map(batch, function put(exists, soul) {
      ...
      s3.PUT(s3.prefix + s3.prenode + soul, node, function (err, reply) {
        _gun2.default.log('s3 put reply', soul, err, reply);
        if (err || !reply) {
  ...
  s3.next = s3.next || _gun2.default.time.is();
  s3.on = s3.on || _gun2.default.on.create();
  ...
  s3.key = s3.key || function (key, soul, cb) {
    ...
    var path = s3.prefix + s3.prekey + key,
        meta = { key: '0.2' },
        rel = {};
    meta[_gun2.default._.soul] = rel[_gun2.default._.soul] = soul = _gun2.default.is.soul(soul) || soul;
    s3.GET(path, function (err, data, text, _) {
      var souls = data || {};
      souls[soul] = rel;
      s3.PUT(path, souls, function (err, reply) {
        _gun2.default.log('s3 key reply', soul, err, reply);
        if (err || !reply) {
